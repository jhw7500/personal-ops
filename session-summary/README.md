...
Claude Code episodic-memory에서 전일 세션을 긁어와 주간 요약을 누적하는 스크립트.

## 동작

- **요약 모드** (인자 없음): 전일 세션 검색 → 요약을 `logs/session-summary.md`에 append
- **로테이트 모드** (`rotate` 인자): `logs/session-summary.md`를 `archive/summary-<지난수>_<이번화>.md`로 이동

## 크론

```cron
10 0 * * 1-5 .../session-summary.sh           # 평일 00:10 요약
